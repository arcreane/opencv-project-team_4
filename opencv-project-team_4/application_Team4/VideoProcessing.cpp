...
// ─── Helper : open video with Windows-safe path ────────────────────────────
bool VideoProcessingWindow::openVideo(cv::VideoCapture& cap, const QString& path)
{
    cap.release();
    QString native = QDir::toNativeSeparators(path);
    QByteArray localPath = native.toLocal8Bit();
    // Try FFMPEG first (handles MP4/MKV/MOV best on Windows)
    if (cap.open(localPath.constData(), cv::CAP_FFMPEG) && cap.isOpened()) return true;
    // Fallback: auto-detect backend
    if (cap.open(localPath.constData()) && cap.isOpened()) return true;
    return false;
}

// ─── Helper : display cv::Mat in a QLabel without growing the window ───────
// KEY FIX: we scale to the CURRENT label size so sizeHint() stays stable.
void VideoProcessingWindow::showPixmap(QLabel* label, const cv::Mat& mat)
{
    if (mat.empty() || !label) return;
    QPixmap px = matToPixmap(mat);
    if (px.isNull()) return;
    QSize sz = label->size();
    if (sz.width() < 2 || sz.height() < 2) sz = QSize(640, 360);
    label->setPixmap(px.scaled(sz, Qt::KeepAspectRatio, Qt::SmoothTransformation));
}
    ...
    , m_startInterface(startInterface)
{
    ...
    // ── FIX: prevent preview labels from growing the window.
    // QLabel::sizeHint() returns the pixmap size, creating a feedback loop where
    // each frame display makes the label (and window) grow. QSizePolicy::Ignored
    // tells the layout to ignore sizeHint() entirely for these labels.
    auto fixLabel = [](QLabel* l) {
        l->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Ignored);
        l->setMinimumHeight(80);
    };
    fixLabel(ui->filterPreviewLabel);
    fixLabel(ui->flowPreviewLabel);
    fixLabel(ui->camPreviewLabel);
    fixLabel(ui->timelapsePreviewLabel);
    fixLabel(ui->stabOrigLabel);
    fixLabel(ui->stabResultLabel);

    m_exportTimer    = new QTimer(this); m_exportTimer->setInterval(30);
    m_flowTimer      = new QTimer(this); m_flowTimer->setInterval(60);
    m_camTimer       = new QTimer(this); m_camTimer->setInterval(33);
    ...
    connect(ui->prevFilterButton,   &QPushButton::clicked, this, &VideoProcessingWindow::onPrevFilterFrame);
    connect(ui->nextFilterButton,   &QPushButton::clicked, this, &VideoProcessingWindow::onNextFilterFrame);
    ...
    connect(ui->prevFlowButton,   &QPushButton::clicked,  this, &VideoProcessingWindow::onPrevFlowFrame);
    connect(ui->nextFlowButton,   &QPushButton::clicked,  this, &VideoProcessingWindow::onNextFlowFrame);
    ...
    connect(ui->prevTimelapseButton,   &QPushButton::clicked,  this, &VideoProcessingWindow::onPrevTimelapseFrame);
    connect(ui->nextTimelapseButton,   &QPushButton::clicked,  this, &VideoProcessingWindow::onNextTimelapseFrame);
    ...
    connect(ui->prevStabButton,  &QPushButton::clicked,  this, &VideoProcessingWindow::onPrevStabFrame);
    connect(ui->nextStabButton,  &QPushButton::clicked,  this, &VideoProcessingWindow::onNextStabFrame);
...
void VideoProcessingWindow::onLoadVideoFilter()
{
    ...
    if (m_filterTotal <= 0) m_filterTotal = 0;
...
void VideoProcessingWindow::onFilterSliderChanged(int value)
{
    ...
    showPixmap(ui->filterPreviewLabel, applyFilter(frame, ui->filterCombo->currentText()));
    ui->frameCountLabel->setText(QString("%1 / %2").arg(value).arg(qMax(0, m_filterTotal - 1)));
}

void VideoProcessingWindow::onPrevFilterFrame()
{
    int v = ui->filterSlider->value();
    if (v > 0) ui->filterSlider->setValue(v - 1);
}
void VideoProcessingWindow::onNextFilterFrame()
{
    int v = ui->filterSlider->value();
    if (v < m_filterTotal - 1) ui->filterSlider->setValue(v + 1);
}
...
void VideoProcessingWindow::onExportFilter()
{
    ...
    int fc = path.endsWith(".mp4", Qt::CaseInsensitive)
...
void VideoProcessingWindow::onExportTimerTick()
{
    ...
    for (int i = 0; i < 5 && m_exportCurrent < m_filterTotal; ++i) {
        cv::Mat frame;
        if (!m_filterCap.read(frame) || frame.empty()) { m_exportCurrent = m_filterTotal; break; }
...
void VideoProcessingWindow::onFlowSliderChanged(int value)
{
    ...
    if (!frame.empty()) showPixmap(ui->flowPreviewLabel, frame);
    ui->flowCountLabel->setText(QString("%1 / %2").arg(value).arg(m_flowTotal - 1));
}

void VideoProcessingWindow::onPrevFlowFrame()
{
    if (m_flowPlaying) { m_flowTimer->stop(); m_flowPlaying = false; ui->playFlowButton->setText("Play"); }
    int v = ui->flowSlider->value();
    if (v > 0) ui->flowSlider->setValue(v - 1);
}
void VideoProcessingWindow::onNextFlowFrame()
{
    if (m_flowPlaying) { m_flowTimer->stop(); m_flowPlaying = false; ui->playFlowButton->setText("Play"); }
    int v = ui->flowSlider->value();
    if (v < m_flowTotal - 1) ui->flowSlider->setValue(v + 1);
}
...
void VideoProcessingWindow::onFlowTimerTick()
{
    ...
    if (!m_prevGrayFlow.empty()) {
        bool farneback = ui->flowCombo->currentText().contains("Farneback");
        cv::Mat flow = farneback
                     ? computeFarnebackFlow(m_prevGrayFlow, curr)
                     : computeLucasKanadeFlow(m_prevGrayFlow, curr);
        // Blend flow with original frame for visibility
        if (farneback && !flow.empty()) {
            cv::addWeighted(curr, 0.4, flow, 0.6, 0, display);
        } else {
            display = flow.empty() ? curr.clone() : flow;
        }
    } else {
        display = curr.clone();
    }

    cv::cvtColor(curr, m_prevGrayFlow, cv::COLOR_BGR2GRAY);
    m_flowFrameIdx++;
    if (!display.empty()) showPixmap(ui->flowPreviewLabel, display);
...
cv::Mat VideoProcessingWindow::computeFarnebackFlow(const cv::Mat& prevGray, const cv::Mat& curr)
{
    ...
    cv::Mat mag, ang, magN;
    cv::cartToPolar(parts[0], parts[1], mag, ang, true);
    cv::normalize(mag, magN, 0, 255, cv::NORM_MINMAX);

    cv::Mat h, s, v;
    ang.convertTo(h, CV_8U, 0.5);
    s = cv::Mat::ones(ang.size(), CV_8U) * 255;
    magN.convertTo(v, CV_8U);
...
cv::Mat VideoProcessingWindow::computeLucasKanadeFlow(const cv::Mat& prevGray, const cv::Mat& curr)
{
    ...
    cv::Mat draw = curr.channels() == 1 ? cv::Mat() : curr.clone();
    if (draw.empty()) cv::cvtColor(curr, draw, cv::COLOR_GRAY2BGR);
    ...
    for (size_t i = 0; i < m_flowPoints.size(); ++i) {
        ...
        cv::circle(draw, nxt[i], 3, cv::Scalar(0, 80, 255), -1);
...
void VideoProcessingWindow::onStartCam()
{
    ...
    m_webcam.set(cv::CAP_PROP_FRAME_HEIGHT, 480);
...
void VideoProcessingWindow::onCamTimerTick()
{
    ...
    cv::Mat cg; cv::cvtColor(frame, cg, cv::COLOR_BGR2GRAY);
    cv::GaussianBlur(cg, cg, cv::Size(5, 5), 0);

    bool motionDetected = false;
    if (!m_prevFrameGray.empty() &&
        m_prevFrameGray.size() == cg.size()) // safety: same dimensions
    {
        cv::Mat diff, thresh;
        cv::absdiff(m_prevFrameGray, cg, diff);
        ...
        if (motionDetected) {
            ...
            if (!m_recording) {
                ...
                m_motionWriter.open(QDir::toNativeSeparators(m_motionRecordPath).toLocal8Bit().constData(),
    ...
    showPixmap(ui->camPreviewLabel, frame);
}
...
void VideoProcessingWindow::onLoadVideoTimelapse()
{
    ...
    setStatus(QString("Video: %1×%2  |  %3 fps  |  %4 frames")
              ...
              .arg(m_timelapseTotal));
}

void VideoProcessingWindow::onTimelapseSliderChanged(int v)
{
    cv::Mat f = grabTimelapsFrame(v);
    if (!f.empty()) showPixmap(ui->timelapsePreviewLabel, f);
    ui->timelapseCountLabel->setText(QString("%1 / %2").arg(v).arg(qMax(0, m_timelapseTotal - 1)));
}

void VideoProcessingWindow::onPrevTimelapseFrame()
{
    int v = ui->timelapseSlider->value();
    if (v > 0) ui->timelapseSlider->setValue(v - 1);
}
void VideoProcessingWindow::onNextTimelapseFrame()
{
    int v = ui->timelapseSlider->value();
    if (v < m_timelapseTotal - 1) ui->timelapseSlider->setValue(v + 1);
}
...
void VideoProcessingWindow::onExportTimelapse()
{
    ...
    if (origFps <= 0) origFps = 25.0;
    int w = (int)m_timelapseCap.get(cv::CAP_PROP_FRAME_WIDTH);
    int h = (int)m_timelapseCap.get(cv::CAP_PROP_FRAME_HEIGHT);
    ...
    setStatus(isTL ? QString("Time-lapse ×%1...").arg(factor)
                   : QString("Slow-motion ×1/%1...").arg(factor));
}
...
void VideoProcessingWindow::onTimelapseExportTick()
{
    ...
    for (int i = 0; i < 5 && m_timelapseExportIdx < m_timelapseTotal; ++i) {
        cv::Mat frame;
        if (!m_timelapseCap.read(frame) || frame.empty()) { m_timelapseExportIdx = m_timelapseTotal; break; }
...
void VideoProcessingWindow::onLoadVideoStab()
{
    ...
    if (!first.empty()) {
        showPixmap(ui->stabOrigLabel, first);
...
{
    if (!m_stabCap.isOpened() || m_stabTotal <= 0) return;
...
{
    for (int i = 0; i < 3 && m_stabProcessIdx < m_stabTotal; ++i) {
        cv::Mat frame;
        if (!m_stabCap.read(frame) || frame.empty()) { m_stabProcessIdx = m_stabTotal; break; }
...
void VideoProcessingWindow::onStabSliderChanged(int value)
{
    ...
    cv::Mat frame; m_stabCap.read(frame);
    if (frame.empty()) return;

    showPixmap(ui->stabOrigLabel, frame);
    if (m_stabPhase == 2 && value < (int)m_stabSmoothed.size())
        showPixmap(ui->stabResultLabel, applyStabCorrection(frame, value));

    ui->stabCountLabel->setText(QString("%1 / %2").arg(value).arg(m_stabTotal - 1));
}

void VideoProcessingWindow::onPrevStabFrame()
{
    int v = ui->stabSlider->value(); if (v > 0) ui->stabSlider->setValue(v - 1);
}
void VideoProcessingWindow::onNextStabFrame()
{
    int v = ui->stabSlider->value(); if (v < m_stabTotal - 1) ui->stabSlider->setValue(v + 1);
}
...
void VideoProcessingWindow::onExportStab()
{
    ...
    int w = (int)m_stabCap.get(cv::CAP_PROP_FRAME_WIDTH);
    int h = (int)m_stabCap.get(cv::CAP_PROP_FRAME_HEIGHT);
    int fc = path.endsWith(".mp4", Qt::CaseInsensitive)
           ? cv::VideoWriter::fourcc('m','p','4','v')
           : cv::VideoWriter::fourcc('M','J','P','G');
...
void VideoProcessingWindow::onStabExportTick()
{
    for (int i = 0; i < 3 && m_stabExportIdx < m_stabTotal; ++i) {
        cv::Mat frame;
        if (!m_stabCap.read(frame) || frame.empty()) { m_stabExportIdx = m_stabTotal; break; }
...
{
    if (prev.empty() || curr.empty()) return {};
    std::vector<cv::Point2f> prevPts;
    cv::goodFeaturesToTrack(prev, prevPts, 200, 0.01, 30);
    if (prevPts.size() < 4) return {};
    ...
    for (size_t i = 0; i < prevPts.size(); ++i)
        if (st[i]) { gp.push_back(prevPts[i]); gc.push_back(currPts[i]); }
    if (gp.size() < 4) return {};
    cv::Mat M = cv::estimateAffinePartial2D(gp, gc);
    if (M.empty() || M.rows < 2 || M.cols < 3) return {};
...
{
    if (frame.empty() || idx < 0 || idx >= (int)m_stabSmoothed.size()) return frame.clone();
...
cv::Mat VideoProcessingWindow::applyFilter(const cv::Mat& frame, const QString& f)
{
    ...
    if (f=="Grayscale") { cv::cvtColor(frame,r,cv::COLOR_BGR2GRAY); cv::cvtColor(r,r,cv::COLOR_GRAY2BGR); }
        ...
        cv::Mat k=(cv::Mat_<float>(3,3)<<0.f,-1.f,0.f,-1.f,5.f,-1.f,0.f,-1.f,0.f);
        cv::filter2D(frame,r,-1,k);
    }
    else r=frame.clone();
    return r;
}
...
QPushButton#prevFilterButton,QPushButton#nextFilterButton,
QPushButton#prevFlowButton,QPushButton#nextFlowButton,
QPushButton#prevTimelapseButton,QPushButton#nextTimelapseButton,
QPushButton#prevStabButton,QPushButton#nextStabButton {
    background-color:#10102a;border-color:#282860;color:#8888cc;
    padding:4px 6px;border-radius:6px;font-size:13px;font-weight:bold; }
QPushButton#prevFilterButton:hover,QPushButton#nextFilterButton:hover,
QPushButton#prevFlowButton:hover,QPushButton#nextFlowButton:hover,
QPushButton#prevTimelapseButton:hover,QPushButton#nextTimelapseButton:hover,
QPushButton#prevStabButton:hover,QPushButton#nextStabButton:hover {
    background-color:#1a1a40;border-color:#4040a0;color:#b0b0ff; }
