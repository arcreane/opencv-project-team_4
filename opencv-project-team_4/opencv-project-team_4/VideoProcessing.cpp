    ...
    , m_startInterface(startInterface)
{
    ...
    // Tab 4
    connect(ui->loadTimelapseButton,   &QPushButton::clicked,
            this, &VideoProcessingWindow::onLoadVideoTimelapse);
    connect(ui->timelapseSlider,       &QSlider::valueChanged,
            this, &VideoProcessingWindow::onTimelapseSliderChanged);
    connect(ui->exportTimelapseButton, &QPushButton::clicked,
            this, &VideoProcessingWindow::onExportTimelapse);
...
{
    m_exportTimer->stop();    m_flowTimer->stop();
    m_camTimer->stop();       m_timelapseTimer->stop();
    m_filterWriter.release(); m_motionWriter.release(); m_timelapseWriter.release();
    m_filterCap.release();    m_flowCap.release();
    m_webcam.release();       m_timelapseCap.release();
    delete ui;
}
...
void VideoProcessingWindow::onExportFilter()
{
    ...
    QString path = QFileDialog::getSaveFileName(this, "Exporter", "output.mp4",
        "MP4 (*.mp4);;AVI (*.avi)");
    if (path.isEmpty()) return;
...
void VideoProcessingWindow::onToggleFlowPlay()
{
    ...
    if (m_flowPlaying) {
        ...
        m_flowTimer->stop(); ui->playFlowButton->setText("Play");
...
void VideoProcessingWindow::onFlowSliderChanged(int value)
{
    ...
    if (!frame.empty()) {
        QSize sz = ui->flowPreviewLabel->size(); QPixmap px = matToPixmap(frame);
        ui->flowPreviewLabel->setPixmap(sz.width()>10 ? px.scaled(sz,Qt::KeepAspectRatio,Qt::SmoothTransformation) : px);
    }
...
void VideoProcessingWindow::onFlowTimerTick()
{
    ...
    if (!m_flowCap.read(currFrame) || currFrame.empty()) {
        ...
        ui->playFlowButton->setText("Play"); m_flowFrameIdx = 0;
        setStatus("Lecture terminée.", StatusType::Success); return;
    }
    cv::Mat result = m_prevGrayFlow.empty() ? currFrame.clone()
...
cv::Mat VideoProcessingWindow::computeLucasKanadeFlow(const cv::Mat& prevGray, const cv::Mat& curr)
{
    ...
    cv::calcOpticalFlowPyrLK(prevGray, currGray, m_flowPoints, nextPts, status, err, cv::Size(21,21), 3);
    std::vector<cv::Point2f> goodPts;
    for (size_t i = 0; i < m_flowPoints.size(); ++i) {
        if (!status[i]) continue;
        cv::arrowedLine(drawFrame, m_flowPoints[i], nextPts[i], cv::Scalar(0,255,0), 2, cv::LINE_AA, 0, 0.3);
...
void VideoProcessingWindow::onStartCam()
{
    ...
    if (!m_webcam.isOpened()) {
        setStatus("Erreur : impossible d'ouvrir la webcam (index 0).", StatusType::Error); return;
    }
...
void VideoProcessingWindow::onCamTimerTick()
{
    ...
    if (!m_prevFrameGray.empty()) {
        ...
        double motionPct = (double)cv::countNonZero(thresh) / (frame.rows * frame.cols) * 100.0;
        ...
        if (motionDetected) {
            ...
            if (++m_noMotionFrames >= 100 && m_recording) {
    ...
    ui->motionIndicatorLabel->setText(
        ...
        : m_recording  ? "● Attente fin de mouvement..."
                       : "● Aucun mouvement");
    ui->motionIndicatorLabel->setStyleSheet(
...
void VideoProcessingWindow::onLoadVideoTimelapse()
{
    QString path = QFileDialog::getOpenFileName(this, "Ouvrir une vidéo", {},
        "Vidéos (*.mp4 *.avi *.mkv *.mov *.wmv)");
    if (path.isEmpty()) return;
    m_timelapseCap.release();
    m_timelapseCap.open(path.toStdString());
    if (!m_timelapseCap.isOpened()) {
        setStatus("Erreur : impossible d'ouvrir la vidéo.", StatusType::Error); return;
    }
...
void VideoProcessingWindow::onTimelapseSliderChanged(int value)
{
    if (!m_timelapseCap.isOpened()) return;
    cv::Mat frame = grabTimelapsFrame(value);
    if (frame.empty()) return;
    QSize sz = ui->timelapsePreviewLabel->size(); QPixmap px = matToPixmap(frame);
    ui->timelapsePreviewLabel->setPixmap(
        sz.width()>10 ? px.scaled(sz,Qt::KeepAspectRatio,Qt::SmoothTransformation) : px);
    ui->timelapseCountLabel->setText(
        QString("%1 / %2").arg(value).arg(qMax(0, m_timelapseTotal - 1)));
}
...
void VideoProcessingWindow::onExportTimelapse()
{
    ...
    // factor = 2 << index: idx0→2, idx1→4, idx2→8, idx3→16
    int    factor  = 2 << ui->timelapseFactorCombo->currentIndex();
    bool   isTL    = (ui->timelapseModeCombo->currentIndex() == 0);

    // Time-lapse: same fps, keep 1/N frames → plays N× faster
    // Slow-motion: fps/N, keep all frames → plays N× slower
    ...
    if (!m_timelapseWriter.open(path.toStdString(), fourcc, outFps, cv::Size(w, h))) {
        setStatus("Erreur : impossible de créer le fichier.", StatusType::Error); return;
    }

    m_timelapseCap.set(cv::CAP_PROP_POS_FRAMES, 0);
    m_timelapseExportIdx = 0;
    ui->timelapseProgressBar->setRange(0, m_timelapseTotal);
    ui->timelapseProgressBar->setValue(0);
    ui->timelapseProgressBar->show();
    ui->exportTimelapseButton->setEnabled(false);
    m_timelapseTimer->start();

    setStatus(isTL
        ? QString("Export Time-lapse ×%1 en cours...").arg(factor)
        : QString("Export Slow-motion ×1/%1 en cours...").arg(factor));
}
...
void VideoProcessingWindow::onTimelapseExportTick()
{
    ...
    for (int i = 0; i < 5 && m_timelapseExportIdx < m_timelapseTotal; ++i) {
        cv::Mat frame;
        if (!m_timelapseCap.read(frame)) { m_timelapseExportIdx = m_timelapseTotal; break; }

        if (isTL) {
            // Time-lapse: write only 1 frame out of every N
            if (m_timelapseExportIdx % factor == 0)
                m_timelapseWriter.write(frame);
        } else {
            // Slow-motion: write every frame (fps is already reduced)
            m_timelapseWriter.write(frame);
        }
    ...
    if (m_timelapseExportIdx >= m_timelapseTotal) {
        ...
        m_timelapseWriter.release();
        ui->timelapseProgressBar->hide();
        ui->exportTimelapseButton->setEnabled(true);
...
cv::Mat VideoProcessingWindow::applyFilter(const cv::Mat& frame, const QString& filterName)
{
    ...
    if      (filterName == "Grayscale") {
        ...
        cv::Mat f; frame.convertTo(f, CV_32FC3);
...
void VideoProcessingWindow::setStatus(const QString& msg, StatusType type)
{
    ...
    case StatusType::Success:
        s = "color:#48d890;background:#071510;border:1px solid #1a5c3a;"
            "border-radius:6px;padding:0 10px;font-size:12px;font-family:'Segoe UI';"; break;
    case StatusType::Error:
        s = "color:#ff5f5f;background:#180808;border:1px solid #5c1a1a;"
            "border-radius:6px;padding:0 10px;font-size:12px;font-family:'Segoe UI';"; break;
    default:
        s = "color:#8888aa;background:#0a0a18;border:1px solid #1c1c38;"
            "border-radius:6px;padding:0 10px;font-size:12px;font-family:'Segoe UI';"; break;
    }
...
QLabel#filterLabel, QLabel#frameLabel, QLabel#flowMethodLabel,
QLabel#flowSpeedLabel, QLabel#flowFrameLabel, QLabel#threshLabel,
QLabel#timelapseModeLabel, QLabel#timelapseFactorLabel, QLabel#timelapseFrameLabel {
    color: #60608a; font-family: 'Segoe UI'; font-size: 12px; background: transparent;
}
QLabel#frameCountLabel, QLabel#flowCountLabel, QLabel#threshValueLabel, QLabel#timelapseCountLabel {
    color: #7070a0; font-family: 'Segoe UI'; font-size: 12px; background: transparent;
}
QLabel#recPathLabel {
    color: #5050a0; font-family: 'Segoe UI'; font-size: 11px; font-style: italic; background: transparent;
}
...
QLabel#camPreviewLabel,    QLabel#timelapsePreviewLabel {
    background-color: #07070f; border: 1px solid #1a1a38;
    border-radius: 8px; color: #30304c; font-family: 'Segoe UI'; font-size: 14px;
}

QSlider::groove:horizontal { background: #141428; height: 4px; border-radius: 2px; }
QSlider::handle:horizontal {
    background: #5050c8; width: 14px; height: 14px; margin: -5px 0; border-radius: 7px;
}
...
QComboBox {
    background-color: #0a0a1c; border: 1px solid #252550; border-radius: 8px;
    color: #c0c0e0; padding: 4px 10px; font-family: 'Segoe UI'; font-size: 13px;
}
QComboBox:hover { border-color: #4848a8; background-color: #0e0e26; }
QComboBox::drop-down { border: none; width: 24px; }
QComboBox QAbstractItemView {
    background-color: #0d0d1c; border: 1px solid #252550; color: #c0c0e0;
    selection-background-color: #28286a; selection-color: #ffffff;
}

QPushButton {
    background-color: #16162e; color: #b0b0d0; border: 1px solid #26264c;
    border-radius: 8px; padding: 6px 14px; font-family: 'Segoe UI'; font-size: 12px; font-weight: 500;
}
...
QPushButton#startCamButton,   QPushButton#loadTimelapseButton {
...
QPushButton#startCamButton:hover,   QPushButton#loadTimelapseButton:hover {
...
QPushButton#exportFilterButton:disabled, QPushButton#exportTimelapseButton:disabled {
